...
<body>
      ...
      <div class="box manual">
        ...
        <div class="system-mode">
            ...
            <div class="mode-row">
                ...
                <button id = "btnAuto" class="mode-btn">Auto</button>
            </div>
        </div>
      </div>
    </div>
  ...
  //Select mode based on clicking Auto/Manual button
  ...
  //Disable Open/Close when in AUTO mode
...
void setupWebServer()
{
  ...
  server.on("/api/mode/auto", HTTP_POST, []()
            { g_mode = MODE_AUTO;  server.send(200, "application/json", "{\"mode\":\"auto\"}"); });
  server.on("/api/mode/manual", HTTP_POST, []()
            { g_mode = MODE_MANUAL;server.send(200, "application/json", "{\"mode\":\"manual\"}"); });
